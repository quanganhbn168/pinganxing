...
@section('title', $category->name . ' - Danh sách Tour')
@section('content')
                ...
                <div class="mb-10 flex flex-wrap gap-3">
                    ...
                    @foreach($categories as $cat)
                        <a href="{{ route('frontend.tours.category', $cat->slug) }}"
                            class="px-5 py-2 rounded-full {{ $cat->id === $category->id ? 'bg-primary text-white shadow-md' : 'bg-white border border-slate-200 text-slate-600 hover:text-primary hover:border-primary' }} text-sm font-bold transition-all">
                            {{ $cat->name }}
                        </a>
                    @endforeach
                </div>
