<div>
    ...
    <section class="content pt-3 pb-2">
        <div class="container-fluid px-2">
            {{-- INFO CARD: Title + Description + Customer --}}
            <div class="card shadow-sm border-0 mb-3">
                <div class="card-body p-3">
                    <div class="d-flex justify-content-between mb-2">
                        <span class="badge badge-light border">#{{ $workOrder->code }}</span>
                        @if($workOrder->priority && $workOrder->priority->value == 'urgent') 
                             <span class="badge badge-danger">GẤP</span>
                        @elseif($workOrder->priority && $workOrder->priority->value == 'high') 
                ...
                <div class="card-body p-3">
                    <div class="tab-content">
                        ...
                        <div class="tab-pane fade {{ $activeTab == 'progress' ? 'show active' : '' }}" id="tab-progress" role="tabpanel">
                            ...
                            {{-- Nút NHẬN VIỆC (Cho trạng thái Pending) --}}
                            @if($workOrder->status === \App\Enums\WorkOrderStatus::PENDING)
                                <div class="alert alert-warning border text-center mb-3 p-3">
                                    <h6 class="font-weight-bold mb-2">Bạn chưa nhận công việc này</h6>
                                    <button wire:click="startJob" wire:loading.attr="disabled" class="btn btn-success btn-block shadow-sm font-weight-bold rounded-pill">
                                        <i class="fas fa-hand-paper mr-2"></i> NHẬN VIỆC NGAY
                                    </button>
                                </div>
                            @endif
                            ...
                            @foreach($tasks as $task)
                                <div class="task-card {{ $task->status === \App\Enums\TaskStatus::COMPLETED ? 'status-border-completed' : ($task->status === \App\Enums\TaskStatus::PROCESSING ? 'status-border-processing' : 'status-border-pending') }}">
                                    ...
                                    {{-- Link to Task Detail (Disabled if Pending) --}}
                                    @if($workOrder->status === \App\Enums\WorkOrderStatus::PENDING)
                                        <div class="d-block p-3 text-dark">
                                            <div class="d-flex justify-content-between align-items-start">
                                                <div style="flex: 1; padding-right: 10px;">
                                                    <h6 class="mb-1 font-weight-bold text-muted" style="font-size: 15px; line-height: 1.4;">
                                                        <i class="fas fa-lock mr-1"></i> {{ $task->report_content }}
                                                    </h6>
                                                </div>
                                                <div class="text-right" style="min-width: 80px;">
                                                    <span class="badge badge-secondary px-2 py-1">Chưa nhận</span>
                                                </div>
                                            </div>
                                        </div>
                                    @else
                                        <a href="{{ route('worker.tasks.detail', $task->id) }}" class="d-block p-3 text-decoration-none text-dark">
                                            <div class="d-flex justify-content-between align-items-start">
                                                <div style="flex: 1; padding-right: 10px;">
                                    ...
                                    <div class="px-3 py-2 border-top bg-light d-flex justify-content-end align-items-center">
                                        ...
                                        {{-- NÚT: ĐÃ XONG (Chỉ hiện khi chưa xong VÀ đã có báo cáo) --}}
                                        @if($task->status !== \App\Enums\TaskStatus::COMPLETED && $task->reports->count() > 0)
                                            <button wire:click="quickFinishTask({{ $task->id }})" 
                                                    wire:confirm="Xác nhận: Đánh dấu nhiệm vụ này là ĐÃ HOÀN THÀNH?"
                                        ...
                                        @if($task->status === \App\Enums\TaskStatus::COMPLETED)
                                            <button wire:click="reopenTask({{ $task->id }})" 
                                                    wire:confirm="Xác nhận: Mở lại nhiệm vụ này?"
                                                    class="btn btn-sm btn-outline-danger font-weight-bold mr-2">
                                                <i class="fas fa-undo"></i> Mở lại
                                            </button>
                                        @endif

                                        <a href="{{ route('worker.tasks.detail', $task->id) }}" class="btn btn-sm btn-primary font-weight-bold">
                                            Vào làm <i class="fas fa-chevron-right ml-1"></i>
                                        </a>
                                    </div>
                                </div>
                            @endforeach
                        ...
                        <div class="tab-pane fade {{ $activeTab == 'materials' ? 'show active' : '' }}" id="tab-materials" role="tabpanel">
                            <table class="table table-sm table-striped mb-0 text-sm">
                                ...
                                <tbody>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="2" class="text-center text-muted py-3">Chưa có vật tư.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
                                ...
                                <div class="px-1">
                                    @foreach($allReports as $rpt)
                                        <div class="card mb-2 shadow-sm border-0">
                                            <a href="{{ route('worker.tasks.detail', $rpt->task_id) }}" class="text-decoration-none">
