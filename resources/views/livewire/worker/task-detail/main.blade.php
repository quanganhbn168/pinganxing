<div>
    {{-- 1. LOAD CSS --}}
    @include('livewire.worker.task-detail.css')

    {{-- 2. HEADER --}}

    {{-- 3. NỘI DUNG CHÍNH --}}
    <div class="app-content-wrapper container-fluid px-2">
        @include('livewire.worker.task-detail.header')
        ...
        @if($activeTab == 'history')
            <div wire:key="tab-history">
                @include('livewire.worker.task-detail.tab-history')
            </div>
        @endif

        @if($activeTab == 'new_report')
            <div wire:key="tab-report">
                @include('livewire.worker.task-detail.tab-report')
            </div>
    ...
    {{-- 6. LOAD JS --}}
    @include('livewire.worker.task-detail.scripts')
</div>
