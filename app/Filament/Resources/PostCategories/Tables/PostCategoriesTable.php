                ...
                DeleteAction::make()
                    ->before(function (DeleteAction $action, $record) {
                        if ($record->posts()->exists()) {
                            Notification::make()->warning()->title('Không thể xóa!')->body('Danh mục đang chứa bài viết.')->send();
                            $action->cancel();
                        }
                    ...
                    DeleteBulkAction::make()
                        ->action(function (Collection $records, DeleteBulkAction $action) {
                            ...
                            foreach ($records as $record) {
                                if ($record->posts()->exists() || $record->children()->exists()) {
                                    $prevented++;
                                } else {
                                    $record->delete();
                                }
                            ...
                            if ($prevented > 0) {
                                Notification::make()
                                    ...
                                    ->body("Không thể xóa {$prevented} danh mục vì đang chứa bài viết hoặc thư mục con.")
                                    ->send();
                            } else {
